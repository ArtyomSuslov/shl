...
void shl_ime_reg_op(enum csinn_dtype_enum dtype, enum csinn_op_enum op_name, void *init, void *exec,
                    void *est, void *cap, void *perf)
{
    ...
    // shl_ime_cb_table[i].shl_cb_value.caps = cap;
    // shl_ime_cb_table[i].shl_cb_value.perf = perf;
    i++;
}
...
{
    // shl_ime_reg_op(CSINN_DTYPE_INT8, CSINN_OP_MATMUL, shl_ime_matmul_init_int8, NULL,
    //                shl_gref_matmul, shl_ime_matmul_cap, shl_ime_matmul_perf);

    shl_ime_reg_op(CSINN_DTYPE_INT8, CSINN_OP_FULLYCONNECTED, shl_ime_fullyconnected_init_int8, NULL,
                   shl_gref_fullyconnected, NULL, NULL);
